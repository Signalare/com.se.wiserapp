...
class WiserGenericDevice extends ZigBeeDevice {
  ...
  async onNodeInit() {
    ...
    if (this.hasCapability('dim')) {
      ...
      if (this.isFirstInit()) {
        this.zclNode.endpoints[this.wiserEndpoint].clusters.levelControl
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    if (changedKeys.includes('onlevel_memory') || changedKeys.includes('onlevel_level')) {
      ...
      const result = await this.zclNode.endpoints[this.wiserEndpoint].clusters.levelControl
    ...
    if (changedKeys.includes('brightness_min')) {
      ...
      const result = await this.zclNode.endpoints[this.wiserEndpoint].clusters.ballastConfiguration
    ...
    if (changedKeys.includes('onoff_offtimer')) {
      ...
      const result = await this.zclNode.endpoints[this.wiserEndpoint].clusters.onOff
