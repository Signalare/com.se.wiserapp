...
class WiserGenericDevice extends ZigBeeDevice {
  ...
  async onNodeInit() {
    ...
    if (this.hasCapability('dim')) {
      ...
      if (this.isFirstInit()) {
        this.zclNode.endpoints[this.mainEndpoint].clusters.levelControl
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    if (changedKeys.includes('onlevel_memory') || changedKeys.includes('onlevel_level')) {
      ...
      const result = await this.zclNode.endpoints[this.mainEndpoint].clusters.levelControl
    ...
    if (changedKeys.includes('brightness_min')) {
      ...
      const result = await this.zclNode.endpoints[this.mainEndpoint].clusters.ballastConfiguration
    ...
    if (changedKeys.includes('onoff_offtimer')) {
      ...
      const result = await this.zclNode.endpoints[this.mainEndpoint].clusters.onOff
    ...
    // Wiser - Backlight Mode
    if (changedKeys.includes('backlight_mode')) {
      const _backlightMode = Cluster.getCluster('wiser').BACKLIGHT_MODE[newSettings['backlight_mode']];
      const result = await this.zclNode.endpoints[this.wiserEndpoint].clusters.wiser
        .writeAttributes({ backlightMode: _backlightMode });
      this.log('[SETTINGS]', '[Write Attribute]', '[Wiser - backlightMode]', `[Value = ${_backlightMode}] Result:\n`, result);
    }
  }
